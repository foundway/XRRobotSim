...
export const SparksEmitter = ({ position, velocity }: { position: Vector3, velocity: Vector3 }) => {
  ...
  const { globalScaleSqrt } = useSceneStore()
  const POS_SPREAD = 0.2 * globalScaleSqrt
  const DIR_SPREAD = 1
  const SPEED_LERP_UPPER_BOUND = 20 * globalScaleSqrt
  const IMPACT_SCALE = 0.5
  const PARTICLE_COUNT_MULTIPLIER = 200 * globalScaleSqrt
  const MIN_SPEED = 0.1 * globalScaleSqrt
  ...
  return (
    <VFXEmitter
      ...
      settings={{
        loop: false,
        duration: 1, 
        nbParticles: MathUtils.inverseLerp(0, SPEED_LERP_UPPER_BOUND, velocity.length()) * PARTICLE_COUNT_MULTIPLIER,
        spawnMode: "burst", 
        delay: 0, 
        particlesLifetime: [0.1, 1],
        size: [0.02 * globalScaleSqrt, 0.04 * globalScaleSqrt],
